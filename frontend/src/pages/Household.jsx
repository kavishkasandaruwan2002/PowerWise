...
const Household = () => {
  ...
  const fetchBudgets = async () => {
    try {
      if (!profile?._id || profile._id === "new") {
        return;
      }
...
await api.post(`/v1/budgets`, payload);
      ...
      setBudgetFeedback({
        type: "success",
        message: "Budget & Alert Engine synchronized.",
      });
      fetchBudgets();
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (profile && profile._id !== "new") {
        ...
        setFeedback({
          type: "success",
          message: "Household synchronized successfully.",
        });
      } else {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10 max-w-7xl mx-auto">
        ...
        <Card className="lg:col-span-1 bg-[#161b2a] border border-slate-800 rounded-[2rem] md:rounded-[3rem] p-6 lg:p-10 overflow-hidden relative shadow-2xl h-fit">
          ...
          <p className="text-slate-500 text-sm font-bold tracking-tight italic mb-10 leading-relaxed">
            Regional coordinates are used to calibrate AI forecasting models
            based on local temperature and solar flux.
          </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10 max-w-7xl mx-auto mt-8 md:mt-10">
          <Card className="lg:col-span-1 bg-[#161b2a] border border-slate-800 rounded-[2rem] md:rounded-[3rem] p-6 lg:p-10 overflow-hidden relative shadow-2xl h-fit">
            ...
            <p className="text-slate-500 text-sm font-bold tracking-tight italic mb-8 leading-relaxed">
              Define your monthly limit. The AI will monitor your consumption
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setBudgetForm({
                        ...budgetForm,
                        percentageThreshold: e.target.value,
                      })
                    }
                  />
                </div>
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    ...
                    placeholder="Optional hard limit"
                    onChange={(e) =>
                      setBudgetForm({
                        ...budgetForm,
                        billThreshold: e.target.value,
                      })
                    }
                  />
                </div>
              ...
              {budgetFeedback.message && (
                <div
                  className={`p-4 rounded-2xl border text-[9px] font-black uppercase tracking-widest text-center ${
              ...
              {budgets.length === 0 ? (
                <div className="p-8 border border-slate-800 border-dashed rounded-[2rem] text-center">
                  <p className="text-slate-500 text-sm font-bold italic">
                    No targets defined in telemetry.
                  </p>
                </div>
              ) : (
                budgets.map((budget) => {
                  const startDate = new Date(
                    budget.startDate || budget.createdAt || new Date(),
                  );
                  const isBudgetPlan = !!budget.monthlyLimit;
                  const bYear = isBudgetPlan
                    ? startDate.getFullYear()
                    : budget.year || 2024;
                  const bMonth = isBudgetPlan
                    ? startDate.toLocaleString("default", { month: "short" })
                    : new Date(bYear, (budget.month || 1) - 1).toLocaleString(
                        "default",
                        { month: "short" },
                      );
                  const displayLimit = isBudgetPlan
                    ? budget.monthlyLimit
                    : budget.targetAmount;
                  ...
                  return (
                    <div
                      key={budget._id}
                      className="flex items-center justify-between p-6 bg-[#0b0e14] rounded-[2rem] border border-slate-800 transition-all hover:border-slate-700"
                    >
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-slate-500 uppercase font-black tracking-widest mt-1">
                            {isBudgetPlan &&
                            budget.alertThresholds?.percentageOfBudget
