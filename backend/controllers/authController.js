...
const register = async (req, res) => {
    ...
    try {
        const { name, email, password, incomeBracket } = req.body; // no adminKey here
...
// ── Admin Registration (separate endpoint, requires adminKey) ───────────────
...
const getMe = async (req, res) => {
    try {
        const user = await User.findById(req.user._id).populate(
            'household', 'name householdType location tariffType incomeBracket budgets'
        );
